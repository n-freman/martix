...
class Matrix:
    def __init__(self, app, font_size=8):
        ...
        self.katakana = np.array([chr(int('0x30a0', 16) + i) for i in range(96)] + [' ' for _ in range(15)])
        self.font = pg.font.SysFont('noto sans jp', font_size, bold=True)
        ...
        self.image = self.get_image('neo.jpg')
    ...
    def get_prerendered_chars(self):
        ...
        prerendered_chars = {
            (char, color): self.font.render(char, True, color) for char in self.katakana for color in char_colors
        }
